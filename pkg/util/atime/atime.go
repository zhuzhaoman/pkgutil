...
// Package atime provides functionality for measuring and displaying time.
//
...
import (
	"context"
	"fmt"
	"github.com/zhuzhaoman/pkgutil/pkg/util/atime/intlog"
	"github.com/zhuzhaoman/pkgutil/pkg/util/atime/utils"
	"os"
	"regexp"
	"strconv"
	"strings"
...
// StrToDate converts string to *Date object. It also supports timestamp string.
...
func StrToDate(str string, format ...string) (*Date, error) {
	if str == "" {
		return &Date{wrapper{time.Time{}}}, nil
	}
	if len(format) > 0 {
		return StrToDateFormat(str, format[0])
	}
	if isTimestampStr(str) {
		timestamp, _ := strconv.ParseInt(str, 10, 64)
		return NewDateFromTimeStamp(timestamp), nil
	}
	var (
		year, month, day     int
	...
	if match = timeRegex1.FindStringSubmatch(str); len(match) > 0 && match[1] != "" {
		...
		return NewDateFromTime(time.Date(0, time.Month(1), 1, hour, min, sec, nsec, local)), nil
	} else {
	...
	return NewDateFromTime(time.Date(year, time.Month(month), day, hour, min, sec, nsec, local)), nil
}

// StrToDateTime converts string to *DateTime object. It also supports timestamp string.
// The parameter `format` is unnecessary, which specifies the format for converting like "Y-m-d H:i:s".
// If `format` is given, it acts as same as function StrToTimeFormat.
// If `format` is not given, it converts string as a "standard" datetime string.
// Note that, it fails and returns error if there's no date string in `str`.
func StrToDateTime(str string, format ...string) (*DateTime, error) {
	if str == "" {
		return &DateTime{wrapper{time.Time{}}}, nil
	}
	if len(format) > 0 {
		return StrToDateTimeFormat(str, format[0])
	}
	if isTimestampStr(str) {
		timestamp, _ := strconv.ParseInt(str, 10, 64)
		return NewDateTimeFromTimeStamp(timestamp), nil
	}
	var (
		year, month, day     int
		hour, min, sec, nsec int
	...
	if match = timeRegex1.FindStringSubmatch(str); len(match) > 0 && match[1] != "" {
		...
		return NewDateTimeFromTime(time.Date(0, time.Month(1), 1, hour, min, sec, nsec, local)), nil
	} else {
		return nil, gerror.NewCodef(gcode.CodeInvalidParameter, `unsupported time converting for string "%s"`, str)
	}
	...
	return NewDateTimeFromTime(time.Date(year, time.Month(month), day, hour, min, sec, nsec, local)), nil
}
...
//func ConvertZone(strTime string, toZone string, fromZone ...string) (*Date, error) {
//	t, err := StrToDate(strTime)
//	if err != nil {
//		return nil, err
//	}
//	var l *time.Location
...
// StrToDateFormat parses string `str` to *Date object with given format `format`.
// The parameter `format` is like "Y-m-d H:i:s".
func StrToDateFormat(str string, format string) (*Date, error) {
	return StrToTimeLayout(str, formatToStdLayout(format))
}

// StrToDateTimeFormat parses string `str` to *DateTime object with given format `format`.
// The parameter `format` is like "Y-m-d H:i:s".
func StrToDateTimeFormat(str string, format string) (*DateTime, error) {
	return StrToDateTimeLayout(str, formatToStdLayout(format))
}

// StrToTimeLayout parses string `str` to *Date object with given format `layout`.
// The parameter `layout` is in stdlib format like "2006-01-02 15:04:05".
func StrToTimeLayout(str string, layout string) (*Date, error) {
...
// StrToDateTimeLayout parses string `str` to *Date object with given format `layout`.
// The parameter `layout` is in stdlib format like "2006-01-02 15:04:05".
func StrToDateTimeLayout(str string, layout string) (*DateTime, error) {
	if t, err := time.ParseInLocation(layout, str, time.Local); err == nil {
...
// ParseDateFromContent retrieves time information for content string, it then parses and returns it
// as *Date object.
...
func ParseDateFromContent(content string, format ...string) *Date {
	var (
		err   error
		match []string
	)
	if len(format) > 0 {
		...
		if len(match) > 0 {
			return NewDateFromStrFormat(match[0], format[0])
		}
	} else {
		if match = timeRegex1.FindStringSubmatch(content); len(match) >= 1 {
			return NewDateFromStr(strings.Trim(match[0], "./_- \n\r"))
...
// ParseDateTimeFromContent retrieves time information for content string, it then parses and returns it
// as *DateTime object.
// It returns the first time information if there are more than one time string in the content.
// It only retrieves and parses the time information with given `format` if it's passed.
func ParseDateTimeFromContent(content string, format ...string) *DateTime {
	var (
		err   error
		match []string
	)
	if len(format) > 0 {
		...
		if len(match) > 0 {
			return NewDateTimeFromStrFormat(match[0], format[0])
		}
	} else {
		if match = timeRegex1.FindStringSubmatch(content); len(match) >= 1 {
			return NewDateTimeFromStr(strings.Trim(match[0], "./_- \n\r"))
		} else if match = timeRegex2.FindStringSubmatch(content); len(match) >= 1 {
			return NewDateTimeFromStr(strings.Trim(match[0], "./_- \n\r"))
		} else if match = timeRegex3.FindStringSubmatch(content); len(match) >= 1 {
			return NewDateTimeFromStr(strings.Trim(match[0], "./_- \n\r"))
